...
#ifndef GYMSIM_HELPER_H
#define GYMSIM_HELPER_H
#include <cmath>
#include <cstdlib>
 ...
 * rand_notrig(5.2,3.0);	//returns deviate with mean=5.2, std. deviation=3.0
 *
 *
...
double rand_notrig(double mu, double sigma) {
...
double rand_trig(double mu, double sigma) {
    static bool deviateAvailable = false;    //	flag
    static double storedDeviate;            //	deviate from previous calculation
    double dist, angle;
...
double fric(double f) {
    if (f == -1) return MAXFLOAT;
    return f;
}


#endif //GYMSIM_HELPER_H
